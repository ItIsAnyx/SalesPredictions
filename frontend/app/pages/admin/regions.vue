...
import AddRegionWindow from "~/components/admin/region/AddRegionWindow.vue";
import RegionRow from "~/components/admin/region/RegionRow.vue";
...
onMounted(async () => {
  ...
  await fetchRegions(currentPage.value);
});
...
const fetchRegions = async (page = 1) => {
  ...
  try {
    ...
    totalRegions.value = res.total_items;
  } catch (error) {
    navigateTo("/");
  }
};
...
<template>
  <section class="p-md md:p-lg max-w-[1280px] w-full mx-auto space-y-lg">
    ...
    <div class="bg-white border border-surface-container rounded-lg shadow-sm">
      ...
      <div class="px-md py-md border-b border-surface-container-low flex items-center justify-between">
        <h3 class="font-bold text-lg text-on-surface">Region Control</h3>
        <div class="flex">
      ...
      <div class="">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-surface-container-low">
            <RegionRow v-for="r in regions"
              :key="r.id"
              :region="r"
              @update-region="fetchRegions"
              />
          </tbody>
        </table>
      </div>
      ...
      <div class="px-md py-sm border-t border-surface-container flex items-center justify-between text-outline text-[11px] font-bold uppercase">
        <p>Showing {{ currentPage === 1 ? currentPage : (currentPage - 1) * 20 }} to {{ currentPage * 20 - 1 > totalRegions ? totalRegions : currentPage * 20 - 1 }} of {{ totalRegions }} regions</p>
        ...
        <div class="flex items-center gap-xs">
          ...
          <button class="w-8 h-8 flex items-center justify-center rounded hover:bg-surface-container-low"
            @click="currentPage++"
            :disabled="currentPage * 20 - 1 > totalRegions"
          >
  ...
  <div v-if="menuAddOpen" @click="toggleMenu" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
        <div @click.stop>
            <AddRegionWindow @close="toggleMenu" @updated="fetchRegions(currentPage)"/>
        </div>
    </div>
</template>